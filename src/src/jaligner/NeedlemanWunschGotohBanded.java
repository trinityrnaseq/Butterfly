 ...
 * 
 * 
 *       extended by bhaas to add banded alignment
 */
package jaligner;
...
public final class NeedlemanWunschGotohBanded {
	...
	private static final byte DIRECTION_INIT = 4;
	
    /**
     * Logger
     */
    private static final Logger logger = Logger
            .getLogger(NeedlemanWunschGotohBanded.class.getSimpleName());

    /**
     * Hidden constructor
     */
    private NeedlemanWunschGotohBanded() {
        super();
    }
    ...
    public static Alignment align(Sequence s1, Sequence s2, Matrix matrix,
        ...
        Cell cell = construct(_s1, _s2, scores, o, e, pointers, lengths, bandwidth);
    ...
    private static Cell construct(Sequence s1, Sequence s2, float[][] matrix,
        ...
        int half_band = Math.max((bandwidth/2), 1);
        ...
        for (int i = 1, k = n; i < m; i++, k += n) { // for all rows
            ...
            for (int j = band_start, l = k + band_start; j < band_end; j++, l++) { // for all columns
            //for (int j = 1, l = k + 1; j < n; j++, l++) { // for all columns

            	//System.out.println(i + ", " + j);
            	
            	// similarity score between the two matching characters in the sequences
                ...
                if (v[j] == f) {
                	...
                	throw new RuntimeException("didn't determine direction");
                }
        ...
        //logger.info("maxi: " + maxi + ", maxj: " + maxj + ", max score: " + maximumScore);
